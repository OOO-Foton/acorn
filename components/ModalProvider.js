...
function GenericModal() {
  const { typeModal, showModal, props, handleClose, setTypeModal, isLoader, setIsLoader, navigationProps } =
  ...
  const Block = ({ typeModal }) => {
    ...
    const pay = async ({ ticket_id, payment_type, amount, method }) => {
      console.log(ticket_id, payment_type, amount, method);
      setIsLoader(true);
      const dataReq = () => {
        if (method === "sberbank") {
          return {
      ...
      return (
        <View>
          ...
          <Button
            ...
            onPress={() => press()}
            title={type === "dobor" ? "Пополнить" : "Оплатить"}
          />
        </View>
      );
    };
    ...
    const bodyTable = () => {
      switch (typeModal) {
        ...
        case "dobor":
          return (
            <InputPay max={props.available} type="dobor" ticket_id={props.ticket_id} prosent={props.percent_sum} />
          );
        case "qrOrAvangard":
          return (
            <View>
